...
> **Market Reference**: Studied **Odoo CRM** (open-source leader: activity calendar with day/week/month views, type-colored entries, click-to-create) and **Pipedrive's Activities calendar** / **HubSpot Meetings** (commercial canon: activity types, linked contacts/deals, conference-platform chips, conflict hints). Adopted: type-driven colors, view switcher incl. agenda, quick create, "effective date" semantics (done → occurred, planned → scheduled). Rejected: external calendar sync (Google/Outlook) — it does not appear in the mockups and belongs to the integrations framework as a future provider package. Drag-and-drop rescheduling was also rejected here and **later reversed** — see Changelog 2026-08-26.
...
- **Drag to create**: pointer-drag on empty grid space draws a dashed preview block and, on release, opens the create editor with the dragged date + start/end prefilled (snapped to 15-minute steps, min 30 minutes). Gated by `customers.interactions.manage`. Drag-to-**reschedule** was originally out of scope and was added later — see Changelog 2026-08-26.
...
### 2026-09-01b — the calendar becomes a second surface over the Task Manager
- **Two domains, deliberately kept two.** The product genuinely holds both concepts and the models are not interchangeable: a `CustomerInteraction` is an *interval* (an instant, a duration, an all-day flag, an RRULE), a `TasksTask` is a *deadline* (`due_date` plus optional `due_time`, no duration, no all-day column, and a recurrence that advances on completion rather than materialising future rows). `CalendarItem` is therefore a discriminated union — `source: 'interaction' | 'task'` — rather than one flattened shape. The compiler now refuses to send a task down the CRM write path, which is what makes "one identity, one source of truth" a checked property instead of a convention.
- **A task on the calendar *is* the task.** `mapTaskToCalendarItem` keeps the task's own id and carries the served record verbatim; nothing is copied into a calendar-side row and no second entity exists. A due-date-only task goes to the all-day lane — what the tasks wire contract already means by `calendarTime: null` — instead of rendering as a 00:00 block that asserts a time the record does not hold. A timed task gets a nominal one-hour slot, matching what the tasks module's own calendar panel draws.
- **Writes go to the Task Manager, through its own endpoints.** A drag writes `{dueDate, dueTime}` to `PATCH /api/tasks/tasks/:id`; completing writes `PATCH /api/tasks/tasks/:id/complete`. Both carry `buildOptimisticLockHeader` and run inside `useGuardedMutation`, so validation, mutation guards, the task command, audit/undo and the `clientBroadcast` event are all the module's own. Nothing about task scheduling is re-implemented on the calendar side.
- **Resize is disabled on tasks, not faked.** A task stores a due moment and no duration, so there is no end edge to drag — pointer and keyboard resize both refuse, while dragging to move still works.
- **Both directions sync through the event bus.** Every task write anywhere in the product (board, list, API, MCP, automation) emits a `clientBroadcast` task event; the calendar subscribes with `useAppEvent('tasks.*')` and refetches, and its own writes emit the same events so the board and lists follow. No polling, no shared cache, no mirroring loop — the calendar renders from the task source of truth and never writes back what it just read.
- **`tz` is sent on completion.** The task API treats `tz` as optional and falls back to UTC, but completion resolves `completedAt` — and a recurring task's next due date — against it. Omitting it recorded the wrong day for anyone completing a task near midnight outside UTC. Caught during verification, not by a test that existed.

#### Why the coupling runs over HTTP and injection, not imports
`tasks/__tests__/moduleGating.test.ts` fails the build if any other core module imports `modules/tasks/`, because that would make disabling tasks break its importer. A first pass did import the module's hooks, `TaskPanel` and status metadata, and the guard caught it. The integration was rebuilt on the two things modules are allowed to share:
- **Data and writes: HTTP.** The calendar reads the tasks module's own windowed `my-tasks/calendar` route (tenant- and organization-scoped, `tasks.view`-gated, capped server-side) and writes through its task routes. The wire shape is declared locally as `calendarTaskPayloadSchema` with `passthrough`, exactly as `calendarInteractionPayloadSchema` already does for the interactions API.
- **The editor: widget injection.** The calendar renders an empty `calendar:task-editor` spot; the tasks module fills it with a widget that mounts its real `TaskPanel` (`features: ['tasks.view']`). One task editor exists in the product, the calendar does not own a second, and disabling tasks empties the spot instead of breaking the page. `TaskPanel` gained optional `defaultDueDate` / `defaultDueTime` so creating from a slot seeds the date the user clicked; creation resolves the Inbox the same way Quick Add does.

#### Permissions and isolation
Task writes are gated on `tasks.edit` and CRM writes on `customers.interactions.manage` — two features, checked separately, so holding one never grants the other. This only hides controls: every endpoint the calendar calls is `requireAuth` + `requireFeatures` server-side, and the task update route spreads the server-derived scope *after* the request body (`{ ...body, ...request.scope, id }`), so a client cannot smuggle a `tenantId`. A caller without `tasks.view` gets a 403 on the task window, which the calendar treats as "no task layer" rather than an error, and still renders its interactions.

#### Known limits
The task layer shows the caller's **assigned** tasks, because that is what the tasks module's calendar endpoint returns; team- and tenant-wide task views would need a new scope on that route and were not invented here. Month-view drag and drop, and recurrence scope editing (this / this-and-following / series), remain unbuilt for both domains.

#### The height the grid actually gets
`Page fill` alone does not clamp this page. The backend shell renders the page inside `<main className="flex-1 …">` with banners as siblings and no definite height anywhere above it, so `h-full` resolves to `auto`, the timed region grows to its full 1152px and the *page* scrolls instead of the grid. The first pass removed the old `min(13 hours, 65vh)` cap on the assumption `fill` would supply the height; it did not, and TC-CAL-008 caught it — `document.elementFromPoint` at 45% down the scroller landed below the fold and returned nothing.

`useAvailableHeight` measures the distance from the grid's top to the bottom of the viewport instead. That keeps the fix inside the calendar rather than changing chrome shared by ~200 backend pages, and it converges: once the grid is clamped the page stops scrolling, so its top stops moving. A floor keeps it readable on short windows, and a 2px dead-band stops a settling layout re-rendering on every frame.

#### Coverage
`lib/calendar/__tests__/taskItem.test.ts` (22 cases) pins identity preservation, local-midnight date parsing, all-day vs timed placement, the drag round-trip, DST-day drops and the timezone contract. `TimeGrid.test.tsx` gains five cases covering task placement, the resize refusal in both input paths, and all-day routing. `tone.test.ts` gains six covering the task treatment. `useCalendarTasks.test.tsx` (10 cases) covers the window boundary, the disabled-module and 403 paths, optimistic drag and its rollback, override retirement and the live-event refresh; `useAvailableHeight.test.tsx` (5) covers the measurement, the floor and the dead-band. Every new regression assertion was verified to fail against the pre-change behaviour. The fixture now derives a task item's calendar status from its task status the way the mapper does, so a test cannot assert on a combination the real projection never produces.

Two integration specs were updated, not weakened. TC-CAL-003 asserted the day view's old `WEEKDAY · DD` header; day view now uses the same stacked weekday/numeral header as week (as a mature calendar does), so the spec asserts the column itself — exactly one `columnheader`, named by its whole date — which additionally pins that Day renders one column and not seven. TC-CAL-008 needed no change once the grid was clamped.


### 2026-09-01 — the grid gets the viewport, and the chrome stops competing with it
- **The page is now viewport-locked.** `backend/calendar/page.tsx` opts into `Page fill="md"` / `PageBody fill="md"`, which already existed in the design system and was simply never used here. That removes the reason the 2026-08-26 pass had to cap the timed region at `min(13 hours, 65vh)`: the ancestors now have a definite height, so the scroll region takes what is left and the calendar fills the window instead of the page scrolling behind a fixed-height grid. Below `md` the page still scrolls naturally, so the grid keeps a `max-h-96` ceiling there — without one the timed region grows to all 24 hours again, which is the exact defect the old cap was working around.
- **Four stacked chrome blocks became two rows.** Navigation (Today, arrows, date label, view switcher, shortcuts, New event) is one bar; scope (category filter, search, filters, settings, range preset, jump-to-date) is the second. The Day/Week/Month/Agenda switcher moved from `CalendarTabs` into `CalendarHeader`, beside the date whose shape it changes. `CalendarFooter` is deleted outright — a permanent rail of eight key chips cost the grid a band of height to teach something a user learns once; `CALENDAR_SHORTCUTS` moved to `ShortcutsDialog`, its only remaining consumer, and the timezone moved into the time-gutter corner where a calendar labels the axis it measures against.
- **Search and filters live on the scope row, not the navigation bar.** Measured against the real stylesheet at the app's actual content width (~1010px at a 1280 viewport), keeping them on row one squeezed the date label to `14 – 20…`. The date is the one fact the whole grid is answering, so the controls that narrow *what* is shown moved down a row, away from the controls that change *when*.
- **The upcoming-cards strip renders in agenda view only.** Day, week and month answer "what is coming" with the grid itself; the strip was ~130px of vertical chrome duplicating it. Conflict surfacing survives in week view through the per-column badge and the warning rings on blocks.
- **Event cards are filled, not tinted.** `lib/calendar/tone.ts` is a new pure module: a live entry takes its calendar's colour as a solid fill and the ink is chosen by measured contrast; a finished or cancelled entry loses the fill entirely and goes outlined, so the grid's colour weight sits on what is still ahead.
  - The ink is a **fixed** off-white/off-black pair, deliberately not `var(--foreground)`/`var(--background)`. The fill is a literal from the tenant's activity-type dictionary and does not change with the theme, so an ink that did flipped underneath it — the first implementation landed near-black on mid-indigo in dark mode at **2.91:1**, below AA. Contrast is a property of the pair, so both halves have to be fixed. Verified in-browser against the compiled stylesheet at 6.29:1 (indigo) and 8.1:1 (amber), identical in both themes.
  - Ink selection **compares the two contrast ratios** rather than testing luminance against a hand-tuned threshold. A threshold is only ever right for the ink pair it was tuned against and picks the worse ink either side of the crossover: `#3b82f6` reads 3.7:1 with white and 4.6:1 with dark, and the threshold set by eye got it backwards.
- **Grid fidelity.** Half-hour rules removed (the 48px hour already halves visibly); the weekend hatch removed; day headers now stack a small weekday token over a large light numeral, with today inverting into a filled disc; the current time replaces the nearest hour label on the axis instead of colliding with it; the now-line and its dot are thicker. Day view uses the same header treatment as week rather than its own `MONDAY · 01` string.
- **Month cell capacity is measured, not fixed.** `rowsPerCellFor` derives how many entries a cell shows from the week row's real height via a `ResizeObserver`, falling back to the previous fixed four until a measurement arrives (and in any environment without layout, tests included). A taller viewport now shows more of a busy day instead of hiding the same overflow count. Month cells also drop the weekend tint, centre the numeral, and label the first of a month with its month name so a row crossing a boundary says which month it crossed into.
- **Deliberately not included:** month-view drag and drop, recurrence scope editing (this / this-and-following / series — still refused rather than faked), and a left sidebar with a mini-month. The sidebar was designed and dropped: at the 1280px viewport the integration specs run against, the app's own nav plus a 256px rail leaves the seven-day grid too narrow to stay readable.

#### Coverage
`lib/calendar/__tests__/tone.test.ts` (22 cases) pins luminance, ink selection across a twelve-colour ramp, AA compliance for every seeded activity-type colour, and the fixed-ink contract. `components/calendar/__tests__/CalendarHeader.test.tsx` (9 cases) picks up the view-switcher contract that moved out of `CalendarTabs`, plus the trailing-slot exclusion. `MonthGrid.test.tsx` gains four `rowsPerCellFor` cases. Each new regression assertion was verified to fail against the pre-change behaviour before being kept. All 25 calendar suites pass (321 cases), and the full ordered gate in `.ai/agentic.config.json` is green including `build:app`, with `lint:ds` at 0 errors.

#### Integration specs
No `TC-CAL-*` selector was changed: every control kept its role and accessible name, `data-calendar-search` stays rendered, `.cursor-cell` still marks the create surface, the conflicted ring class is unchanged, and the `Keyboard shortcuts` button moved from the footer to the navigation bar under the same name. The specs were not executed — the harness needs a database this environment cannot authenticate against (see `.ai/qa/AGENTS.md`).
### 2026-08-26 — Google-Calendar-grade layout, all-day lane and rescheduling
- **Drag-to-reschedule is no longer out of scope.** The original decision (Market Reference, and the Drag-to-create bullet) rejected it because it does not appear in the mockups. It is reinstated deliberately: the calendar is now expected to behave like a mature calendar product, and a grid you can read but not manipulate is the single largest gap against that bar. Drag-to-move, edge-resize and all-day⇄timed conversion all write through the **existing** `PUT /api/customers/interactions` with `buildOptimisticLockHeader` — no new endpoint, no new ACL feature, no schema change.
- **A pure domain layer now owns all date and geometry maths** (`lib/calendar/time.ts`, rewritten `lib/calendar/layout.ts`). Views became renderers. Positions derive from **wall-clock minutes**, not elapsed milliseconds, so 09:00 sits on the 09:00 line on the 23- and 25-hour days a DST transition produces; the previous `setHours(0,0,0,0)` + `setMinutes(n)` arithmetic in `grid.ts` was silently wrong across those boundaries.
- **Month view dropped multi-day events after their first day** — `groupItemsByDay` bucketed every entry by `item.start` alone, so a Mon–Thu event rendered only on Monday. Month rows now pack per-week segments carrying `continuesBefore`/`continuesAfter`, so a span renders as one continuous bar and a week-boundary crossing becomes two marked segments. This was a live defect, not a missing feature.
- **All-day entries became representable.** `mapInteractionToCalendarItem` previously forced `end = endOfDay(start)` for every all-day item, so a multi-day booking could not exist in the model. All-day items now run to the exclusive midnight closing their last day, derived from `durationMinutes`. The lane renders always (so it stays a drop target when empty), packs bars into non-colliding rows, and collapses past two rows behind an overflow control.
- **Overlap layout gained a rightward expansion pass.** Greedy column packing alone shrank every member of a six-way cluster to a sixth of the width; an event now widens into any column to its right holding nothing concurrent, so only genuinely simultaneous events share the split.
- **Recurrence covers monthly, yearly, intervals and exceptions.** `parseRecurrenceRule` previously returned `null` for anything but `DAILY`/`WEEKLY`, so a monthly series silently rendered as a single event. Month-end clamping follows the approach already proven in `tasks/lib/calendar.ts` (31st → 28/29 Feb → 30 Apr) rather than importing across the module boundary. Stepping is frequency-aware, so a yearly rule no longer walks day by day. Unparseable rules still degrade to the single stored event.
- **Editing one occurrence of a series is refused, not faked.** An expanded occurrence carries the master's id, so persisting a drag would rewrite the whole series. Rather than corrupt a schedule, the drag is blocked with an explanation until per-occurrence scope editing exists (this event / this and following / entire series).
- **A current-time indicator exists for the first time**, driven by one shared minute-tick that aligns to the minute boundary and pauses while the tab is hidden. It replaces a `Date.now()` read that happened during render — impure, never refreshed, and defeating memoisation. Default scroll centres on the current time when today is in view, instead of a hardcoded 08:00.
- **The timed region was not scrolling.** Replacing the original fixed `maxHeight` with `flex-1` left it unbounded — the calendar's ancestors have no definite height — so it grew to all 24 hours and pushed the *page* into a scroll. Capped at `min(13 hours, 65vh)`, which also makes the default-scroll behaviour observable.
- **Consistency:** week and month render spanning entries through one shared `CalendarBar` and open the same `EventPeekPopover`; loading uses the DS `Skeleton` primitive rather than a hand-rolled pulse; `resolveJoinUrl` and the lane-packing pass each exist once. `packOverlaps` was removed as dead code once `layoutTimedDay` replaced it.
- **Deliberately not included:** tasks-vs-events representation, recurrence scope editing and its UI, a lightweight quick-create popover, and configurable week start / working hours. Each is tracked separately; none is blocked by this change.

#### Integration coverage
TC-CAL-002, TC-CAL-008 and TC-CAL-010 were updated, not weakened. Day columns now stack an uppercased weekday token above the numeral and expose the **whole date as the column's accessible name**, so those specs assert `columnheader` names rather than a single `"27 SAT"` text node — better for screen readers and a stabler hook. TC-CAL-008's scroller locator follows the timed region's move to vertical-only overflow. All 15 calendar specs pass with no retries.

### 2026-08-25 — every calendar toggle is one segmented control
- `CalendarTabs` now renders the All Scheduled / Meetings / Events filter with `SegmentedControl` — the same DS primitive as the Day/Week/Month/Agenda switcher beside it — instead of `Tabs` + underline `TabsTrigger`s. The filter narrows the items shown in the single view area rather than swapping content panels, so the segmented control's mutually-exclusive-view-state contract is the correct one; `Tabs` additionally promised a `tabpanel` per option that never existed.
- Behavior is unchanged: same three scopes, same counts (`Meetings (n)` / `Events (n)`), same leading icons (now via the primitive's `icon` slot, still `aria-hidden`), same `onTabChange` payloads. The `CalendarTab` type, `CalendarTabsProps` and `CalendarScreen` state are untouched. The count is `text-current opacity-70` rather than `text-muted-foreground` so it stays legible once the segment is filled.
- ARIA moves from `tab`/`tablist` to `radio`/`radiogroup`, so the filter needs its own group name: new i18n key `customers.calendar.tabs.label` ("Calendar category") across en/de/es/ko/pl, distinct from `customers.calendar.views.label` ("Calendar view"). TC-CAL-002 and TC-CAL-004 were updated to scope their radio lookups to that group, and now also assert the checked segment after each switch.
- **The module-local `editor/SegmentGroup.tsx` is deleted.** It was a second segmented toggle built from `Button` + `aria-pressed`, with its own indigo selected fill, no radio ARIA and no sliding pill. Its three call sites — the editor type switcher, the Customization modal's conflict scope, and the repeat "Ends" selector — now use `SegmentedControl` with the new `fullWidth` variant, so every toggle in the calendar (and in the product) is the same component. TC-CAL-005 and TC-CAL-007 moved from `group`/`button`/`aria-pressed` to `radiogroup`/`radio`/`toBeChecked`.
- New unit suite `components/calendar/__tests__/CalendarTabs.test.tsx` (7 cases) pins the radiogroup structure, the icon-excluded accessible names, checked state and both change callbacks.

#### `SegmentedControl` primitive changes (`packages/ui/src/primitives/segmented-control.tsx`)
- **Geometry.** The track is `items-stretch` and items no longer carry `h-7`/`h-6`; they derive height from the track's content box. The selected pill is therefore inset by the track's `p-0.5` on all four sides (2px), instead of 1px vertically against 2px horizontally. Labels stay centred by the item's own `inline-flex items-center justify-center`.
- **Selected colour.** The pill is `bg-sidebar` with `text-sidebar-foreground` ink, so "selected" is the same navy as the app sidebar everywhere in the product. Label colour transitions over `duration-200` — the pill's travel window — so ink and fill arrive together rather than the text snapping to its selected colour while the pill is still in transit; `motion-reduce:transition-none` opts out via CSS (not `useReducedMotion`, which would differ between SSR and hydration).
- **New `fullWidth` variant** — track spans its container, items get `flex-1 basis-0` for equal shares regardless of label length. This is what replaces `SegmentGroup`'s reason to exist. `display` moved into the variant so the `flex`/`inline-flex` choice never depends on Tailwind's emission order.
- **New `icon` slot on `SegmentedControlItem`** — rendered `aria-hidden` and outside the label's width-reservation grid, so it does not double-render and the accessible name stays the label.
- Docs (`.ai/ui-components.md`) and the design-system gallery gained `with-icons` and `full-width` variants plus two MUST rules (never build a parallel `aria-pressed` toggle; pass icons via `icon`, not `children`); the derived harness inventory was refreshed to match. Primitive suite is 23 cases.
