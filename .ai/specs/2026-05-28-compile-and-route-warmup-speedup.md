...
|-------------|-------------|
| Enable `turbopackMinify` / `serverMinification` | Off in dev because enabling them slows the very compile path this spec targets. **Correction (2026-08-25): they are off in *every* environment, production included, and that is load-bearing — not a dev-speed preference.** MikroORM's legacy decorators key entity metadata by class name, the minifier mangles distinct entity classes to the same identifier, and their metadata buckets then collide (`comments` is `@OneToMany` in `customers/data/entities.ts:115,383` but a scalar `@Property` in `sales/data/entities.ts:425,886` and `workflows/data/entities.ts:540`), so `next build` dies with `MetadataError: Multiple property decorators used on 'I.comments'`. Turbopack applies `turbopackMinify` to the server graph too, so disabling only `serverMinification` is not sufficient. See the comment in `apps/mercato/next.config.ts`. |
...
### 2026-08-25
- Corrected the "Enable `turbopackMinify` / `serverMinification`" row in Alternatives Considered. It described the flags as a dev-only compile-speed choice, which read as "production is minified". It is not: both are set unconditionally in `apps/mercato/next.config.ts`, so production client chunks ship unminified (~62 MB across `static/chunks`; measured headroom ≈40% gzip / ≈69% raw). Verified by building with them gated to dev — `next build` fails collecting page data with `MetadataError: Multiple property decorators used on 'I.comments'`, because MikroORM's legacy decorators key metadata by class name and minification collapses distinct entity classes onto the same mangled identifier. Gating only `serverMinification` does not help; Turbopack minifies the server graph via `turbopackMinify`. `next.config.ts` now carries the full reasoning inline so the flags are not "cleaned up" by a future reader. Lifting this needs the entities migrated off the legacy decorators to the Stage-3 (`Symbol.metadata`) ones, which are per-class and immune to name mangling.
