...
- **Customers** – `customers.company.detail:tabs` and `customers.person.detail:tabs` for tabs; `detail:customers.<entity>:header`, `:status-badges` and `:footer` around the detail form, and `crud-form:customers.<entity>:fields` for fields inside it. The v1-only `customers.<entity>.detail:details` spots were removed with the v1 detail pages.
