...
describe('module-facts BC resolve guard (T2)', () => {
  ...
  it('preserves stable v1 extension arrays while exposing corrected v2 facts separately', () => {
    // Upstream also asserted the v1→v2 `component-override` mode correction
    // ('replace' → 'wrapper') using the enterprise `security` module's
    // `section:auth.login.form` contribution. That module is commercially
    // licensed and is not part of this fork (see NOTICE.md), and no OSS module
    // reproduces a v1/v2 mode divergence — verified by extracting both contract
    // versions across every discovered module and finding zero differing
    // component-override contributions. The recovered-contribution assertion
    // below still exercises the v1/v2 divergence via a core module.
