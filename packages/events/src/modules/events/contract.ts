...
/**
 * Public contract for `events`.
 *
 * Everything under `data/`, `lib/`, `services/`, `components/` and `api/` is
 * private: the package exports map sends `./modules/events/*` to null, so a
 * cross-module deep import fails to resolve at compile time rather than in review.
 *
 * Empty because no other module consumes events today. Publishing a surface
 ...
 *
 *   commands:   ['events.<entity>.<action>']   // cross-module writes, via the command bus
 *   readModels: ['events:<entity>']            // cross-module reads, via the query engine
 *
