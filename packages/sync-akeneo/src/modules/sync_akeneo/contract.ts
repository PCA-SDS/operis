...
/**
 * Public contract for `sync_akeneo`.
 *
 * Everything under `data/`, `lib/`, `services/`, `components/` and `api/` is
 * private: the package exports map sends `./modules/sync_akeneo/*` to null, so a
 * cross-module deep import fails to resolve at compile time rather than in review.
 *
 * Empty because no other module consumes sync_akeneo today. Publishing a surface
 ...
 *
 *   commands:   ['sync_akeneo.<entity>.<action>']   // cross-module writes, via the command bus
 *   readModels: ['sync_akeneo:<entity>']            // cross-module reads, via the query engine
 *
