...
| `commands/scope` | When a command handler needs the actor's tenant/organization scope, or must assert a target is inside it. `resolveCommandActorScope(ctx)` returns `{ tenantId, organizationId, requireTenant, requireOrganization }` — the require flags matter because a privileged actor is genuinely unrestricted while an ordinary actor with no organization must still be pinned to `organization_id IS NULL`. `applyActorScopeToWhere(where, scope)` writes that into a `FilterQuery`, `actorScopeForDecryption(scope)` narrows it for `findWithDecryption`. MUST be used instead of re-deriving the scope per module — planner and staff each carried a byte-identical private copy | `@open-mercato/shared/lib/commands/scope` — `resolveCommandActorScope`, `explicitCommandActorScope`, `applyActorScopeToWhere`, `actorScopeForDecryption`, `ensureTenantScope`, `ensureOrganizationScope`, `ensureSameScope` |
...
| `db/pg-errors` | When classifying a Postgres failure — `isUniqueViolation(err, constraintName?, { matchMessage? })` walks the `cause`/`previous`/`driverError` chain and checks `code`/`sqlState`, so it catches the violation whichever wrapper surfaced it. Twelve modules each hand-rolled a different subset and so each missed cases the others caught. Message matching is OPT-IN: keying off error text misclassifies unrelated failures that merely quote a constraint. Also exports `isTransientDbError` | `@open-mercato/shared/lib/db/pg-errors` |
...
| `delivery/` | When scheduling delivery/retry attempts — exponential backoff with jitter, shared by the push delivery worker and outbound webhook delivery | `@open-mercato/shared/lib/delivery/retry` (`calculateBackoffDelayMs`) |
...
| `date/normalize` | When mapping a stored date onto an API response or command input. Two contracts, deliberately both kept: `toIsoOrNull` answers `null` for anything unparseable, `toIsoOrEcho` hands the original string back — around sixty hand-written copies had settled into one or the other, and within a single module the API layer echoed while the command layer nulled. Prefer `toIsoOrNull` in new code | `@open-mercato/shared/lib/date/normalize` — `toIsoOrNull`, `toIsoOrEcho`, `toDateOrNull`, `toDateOnlyIso` |
| `date/` | When feeding a date into an `<input type="date">` — normalizes `Date`/ISO/`YYYY-MM-DD` to `YYYY-MM-DD`, short-circuiting values that are already date-only so an offset-bearing timestamp does not shift a day | `@open-mercato/shared/lib/date/format` — `toDateInputValue(value)` |
| `http/errorMessage` | When rendering a failed API response — `toErrorMessage` walks `error`/`message`/`detail`/`details`/`description` through arrays and nested objects. In `shared` because `ui` needs it and cannot depend on `core` | `@open-mercato/shared/lib/http/errorMessage` |
| `http/readJsonSafe` | When reading a JSON body off a `Request`/`Response`/string — the reader the root `AGENTS.md` mandates over `.json().catch(...)`. Returns the fallback on a JSON *parse* failure; a failed body read still rejects, so callers that want a fallback there handle it themselves (see `apiCall`) | `@open-mercato/shared/lib/http/readJsonSafe` |
| `json/` | When snapshotting a JSON column before mutating it, or coercing an unknown into an object | `@open-mercato/shared/lib/json/cloneJson`, `@open-mercato/shared/lib/json/parseObjectLike` |
...
| `i18n/persistLocale` | When a locale switcher writes the viewer's choice back to their session — one implementation behind the backend profile dropdown, the public language switcher and the checkout pay page | `@open-mercato/shared/lib/i18n/persistLocale` — `persistLocalePreference(locale)` |
...
| `string.ts` | When parsing comma-separated lists, or trimming a string to blank-means-nothing. `normalizeOptionalString` answers `null`, `trimToUndefined` answers `undefined` (six modules each had a private copy of the former); `toOptionalString` is neither — it leaves the value untrimmed and coerces numbers | `@open-mercato/shared/lib/string` — `normalizeOptionalString`, `trimToUndefined`, `parseCommaSeparatedList`, `toOptionalString` |
| `units/fileSize` | When rendering a byte count for display — `formatFileSize(bytes)`. Five near-copies disagreed (1500 B read as `1 KB` or `1.5 KB`; a non-finite value as `NaN B`, `—` or `''`). All five call sites now share it | `@open-mercato/shared/lib/units/fileSize` |
| `validation/` | When validating a field whose rule is shared across modules — email, ISO-4217 currency code, `page`/`pageSize`. MUST be preferred over hand-writing the rule again: the same concept was being validated up to six different ways across modules, and an unconstrained money amount is worse than inconsistent — the gateway adapters convert with `Math.round(amount * 100)`, so `0.001` created a session that charged nothing. Pass `scale` on any amount that becomes minor units. which is how a client ends up accepting what the server rejects. Each factory takes the options real call sites need (`message` for an i18n key, `normalizeCase` for currency, `maxPageSize` for a deliberately higher ceiling) so adopting it does not change what a route accepts. `__tests__/equivalence.test.ts` pins each schema against the hand-written one it replaced. **Adoption is partial** — ~40 email schemas, 13 weak currency checks and every `page`/`pageSize` are still hand-written, so check rather than assume. `emptyStringToNull` is the preprocessor that makes a field form-clearable; `LOOSE_EMAIL_PATTERN` is a fast client/classifier regex, never an enforcement point | `@open-mercato/shared/lib/validation` — `emailSchema`, `normalizeEmail`, `LOOSE_EMAIL_PATTERN`, `emptyStringToNull`, `currencyCodeSchema`, `moneyAmountSchema`, `moneyDecimalStringSchema`, `paginationQuerySchema`, `MAX_PAGE_SIZE`, `MONEY_MAX_AMOUNT` |
| `testing/` | When bootstrapping tests — register only what the test needs; `pathAssertions` normalizes `\`-vs-`/` so a path assertion is platform-independent | `@open-mercato/shared/lib/testing/bootstrap`, `@open-mercato/shared/lib/testing/pathAssertions`, `@open-mercato/shared/lib/testing/renderWithProviders` |
...
| Search config types (`SearchModuleConfig`); `assertSearchTenantContext(ctx, moduleId)` is the descriptor tenant guard (four modules each had a copy) | `@open-mercato/shared/modules/search` |
