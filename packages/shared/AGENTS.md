...
| `validation/` | When validating a field whose rule is shared across modules — email, ISO-4217 currency code, `page`/`pageSize`. MUST be preferred over hand-writing the rule again: the same concept was being validated up to six different ways across modules, and an unconstrained money amount is worse than inconsistent — the gateway adapters convert with `Math.round(amount * 100)`, so `0.001` created a session that charged nothing. Pass `scale` on any amount that becomes minor units. which is how a client ends up accepting what the server rejects. Each factory takes the options real call sites need (`message` for an i18n key, `normalizeCase` for currency, `maxPageSize` for a deliberately higher ceiling) so adopting it does not change what a route accepts. `__tests__/equivalence.test.ts` pins each schema against the hand-written one it replaced | `@open-mercato/shared/lib/validation` — `emailSchema`, `normalizeEmail`, `currencyCodeSchema`, `moneyAmountSchema`, `moneyDecimalStringSchema`, `paginationQuerySchema`, `MAX_PAGE_SIZE`, `MONEY_MAX_AMOUNT` |
