...
| `http/errorMessage` | When rendering a failed API response — `toErrorMessage` walks `error`/`message`/`detail`/`details` through arrays and nested objects. In `shared` because `ui` needs it and cannot depend on `core` | `@open-mercato/shared/lib/http/errorMessage` |
