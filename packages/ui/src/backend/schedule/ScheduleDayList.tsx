...
function formatDayLabel(day: Date, weekday: 'long' | 'short'): string {
  return day.toLocaleDateString(undefined, { weekday, month: 'short', day: 'numeric' })
}
...
/** The props both public schedule views expose. */
export type ScheduleDayListProps = {
  items: ScheduleItem[]
  range: ScheduleRange
  timezone?: string
...
type ScheduleDayListLayout = {
  /** Class applied to the day-card container — stacked for agenda, columns for grid. */
  containerClassName: string
  /** `Intl.DateTimeFormat` weekday width for the day heading. */
  weekday: 'long' | 'short'
}

/**
 * One day-card renderer behind both `ScheduleAgenda` and `ScheduleGrid`.
 *
 * The two used to be 136-line files that differed only in their container class
 * and the weekday width of the day heading, so every fix to item rendering,
 * recurrence expansion or the empty-day state had to be made twice.
 */
export function ScheduleDayList({
  ...
  className,
  layout,
}: ScheduleDayListProps & { layout: ScheduleDayListLayout }) {
  const t = useT()
  ...
  return (
    <div className={cn(layout.containerClassName, className)}>
      {days.map((day) => {
