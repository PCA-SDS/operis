...
describe('CrudForm validation state', () => {
  it('renders each custom field error through exactly one declared owner', async () => {
    ...
    await waitFor(() => {
      expect(wrapperOwnedField?.querySelector('.text-xs.font-medium.text-status-error-text')).toHaveTextContent('This field is required')
      expect(getByTestId('component-owned-error')).toHaveTextContent('This field is required')
    })

    expect(componentOwnedField?.querySelector('.text-xs.font-medium.text-status-error-text')).toBeNull()
  })
  ...
  it('clears corrected field errors immediately without dropping unrelated errors', async () => {
    ...
    await waitFor(() => {
      expect(nameField?.querySelector('.text-xs.font-medium.text-status-error-text')).toHaveTextContent('This field is required')
      expect(gatewayField?.querySelector('.text-xs.font-medium.text-status-error-text')).toHaveTextContent('This field is required')
    })
    ...
    await waitFor(() => {
      expect(nameField?.querySelector('.text-xs.font-medium.text-status-error-text')).toBeNull()
      expect(gatewayField?.querySelector('.text-xs.font-medium.text-status-error-text')).toHaveTextContent('This field is required')
    })
