...
1. **NEVER use raw `<button>` or `<input type="checkbox">`** — always use the primitives. Native checkboxes keep an `accent-color` safety net; new code MUST use `Checkbox`.
...
6. **Filled selection controls (`Checkbox`/`Radio`/`Switch`) use `--primary`.** `--accent-strong` is the *interactive* accent (links, tabs, sort, slider) — never a filled selection.
