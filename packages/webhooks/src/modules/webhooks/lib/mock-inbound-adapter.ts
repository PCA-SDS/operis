...
import type { WebhookEndpointAdapter } from './adapter-registry'

/**
 * A network-free inbound webhook endpoint used ONLY by tests and local development.
 *
 * It lived in the app's `example` module until that module left the build in
 * `d108ad27`, which took `/api/webhooks/inbound/mock_inbound` with it. That URL
 * is not incidental: the outbound delivery specs point their webhooks AT it, so
 * its absence made every delivery 404, retry, exhaust and land as `expired` —
 * six failures that read as a broken delivery pipeline rather than a missing
 * fixture. A test fixture for this module belongs in this module.
 *
 * **Never registered at import.** `ensureMockInboundAdapterRegistered()` is the
 * only entry point and no-ops unless `OM_ENABLE_MOCK_INBOUND_WEBHOOK` is set.
 */
