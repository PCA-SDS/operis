...
MikroORM's identity map can silently discard pending scalar changes when a query runs on the same `EntityManager` before an explicit `em.flush()`, and multiple flushes without a transaction risk partial commits.

- Use `withAtomicFlush(em, phases, options)` from `@open-mercato/shared/lib/commands/flush` when a command mutates entities across phases that include queries on the same `EntityManager`.
- **NEVER** run `em.find` / `em.findOne` between scalar mutations and `em.flush()` on the same `EntityManager` without it.
- Pass `{ transaction: true }` when atomicity matters, and `{ label: '<module>.<command>' }` so the pending-changes guard's warning is actionable.
- Keep `emitCrudSideEffects` / `emitCrudUndoSideEffects` and cache invalidation **outside** the block — they must fire only after the write commits. Applies to `undo` handlers too.
- Prefer `runCrudCommandWrite` when a command writes an entity, its custom fields and CRUD/index side effects together; it owns the fork, the flush boundary and the side-effect order.

Worked examples, the commit-boundary guarantee and the wrong/right pair: [`.ai/docs/entity-flush-safety.md`](../../.ai/docs/entity-flush-safety.md).
...
- Authoring rules (generated SQL, snapshot drift, hand-written migrations): § Entity Schema And Migration Workflow.
...
- UUID PKs, explicit FKs, junction tables for M2M. **A PK not named `id` breaks `yarn initialize`** (the reindexer selects `b.id`): natural key ⇒ surrogate `id` + `@Unique`.
