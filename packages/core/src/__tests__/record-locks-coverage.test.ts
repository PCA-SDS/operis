...
const RECORD_LOCKS_DECISIONS: Record<string, RecordLockDecision> = {
  ...
  // --- invoice ---
  'appointments:Appointment': { status: 'enabled', resourceKind: 'appointments.appointment', reason: 'enabled — PATCH /api/appointments/[id] loads the tenant-scoped appointment and enforces the command-level updated_at floor before the status change; the staff detail page already sends the version header and surfaces the 409 via surfaceRecordConflict.' },
  'appointments:AppointmentStatus': { status: 'enabled', resourceKind: 'appointments.status', reason: 'enabled — CAP staff parity exposes PATCH/DELETE /api/appointments/statuses/[id] for the tenant status catalog; both load the tenant-scoped row and enforce the command-level updated_at floor, and AppointmentStatusSettings sends the version header on edit and delete.' },
  'email:EmailTemplate': { status: 'enabled', resourceKind: 'email.template', reason: 'enabled — makeCrudRoute drives PUT/DELETE through email.templates.update/delete, both of which load the tenant+org-scoped row and call assertOptimisticLock against the expected updated_at; the builder page sends the version header on save and delete.' },
  'email:EmailAccountingDefaults': { status: 'enabled', resourceKind: 'email.accounting_defaults', reason: 'enabled — the custom PUT /api/email/accounting-defaults loads the scoped row and calls assertOptimisticLock on the expected_updated_at token the settings page sends. No CRUD reader is registered for this entity, so the generic header path is inert and the body token is the enforced one.' },
  'invoice:InvoiceCompany': { status: 'enabled', resourceKind: 'invoice.company', reason: 'enabled — CAP-003 exposes a custom PATCH /api/invoice/partners/[id] write surface that loads the scoped live partner, enforces the command-level updated_at floor, and runs route mutation guards before updating default_due_days only.' },
  ...
  'invoice:Invoice': { status: 'enabled', resourceKind: 'invoice.invoice', reason: 'enabled — CAP-001 exposes manual invoice commands and the dedicated due-date command; custom routes load the scoped live invoice, enforce the command-level updated_at floor, and run route mutation guards before writing tenant-owned invoice data.' },
