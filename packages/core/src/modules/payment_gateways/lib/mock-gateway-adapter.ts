...
/**
 * A network-free payment gateway used ONLY by tests and local development.
 *
 * It lived in the app's `example` module until that module was dropped from the
 * build (`d108ad27`, "drop development-only modules"), which took the `mock`,
 * `mock_usd` and `mock_processing` providers with it and left 29 integration
 * specs asserting against a gateway nothing registered any more. A test fixture
 * for this module belongs in this module, not in a product module that can be
 * switched off.
 *
 * **It is never registered at import.** `ensureMockGatewayRegistered()` is the
 * only entry point and no-ops unless `OM_ENABLE_MOCK_PAYMENT_GATEWAY` is set —
 * the same shape `push_notifications` uses for its stub channel adapter. A mock
 * gateway that reached production would accept payments that never happened.
 */
