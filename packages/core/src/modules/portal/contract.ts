...
/**
 * Public contract for `portal`.
 *
 * Customer-portal shell wiring. Contributes navigation, owns no data.
 *
 * Everything under `data/`, `lib/`, `services/`, `components/` and `api/` is
 * private: `packages/core/package.json` maps `./modules/portal/*` to null, so a
 * cross-module deep import fails to resolve at compile time rather than in review.
 *
 * This contract is deliberately empty. No other module consumes portal today, and
 ...
 *
 *   commands:   ['portal.<entity>.<action>']   // cross-module writes, via the command bus
 *   readModels: ['portal:<entity>']            // cross-module reads, via the query engine
 *
