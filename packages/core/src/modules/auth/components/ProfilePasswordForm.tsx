...
const logger = createLogger('auth').child({ component: 'profile-password-form' })
...
export type ProfilePasswordFormProps = {
  /** Heading shown above the form. The two routes that host this differ only here. */
  title: string
  /** Extra classes for the card, so a full-width host can opt out of the max width. */
  className?: string
}

/**
 * The profile password form, shared by `/backend/auth/profile` and
 * `/backend/profile/change-password`. Both routes are intentional entry points
 * with their own breadcrumbs, but they previously carried ~180 duplicated lines
 * of identical schema, load and submit logic that could drift apart.
 */
export function ProfilePasswordForm({ title, className }: ProfilePasswordFormProps) {
  const t = useT()
