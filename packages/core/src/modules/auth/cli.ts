...
// `--roles` selects which roles are CREATED in the new tenant. It does NOT scope the
// primary user: `setupInitialTenant` assigns the primary user `primaryUserRoles`, which
// this command deliberately leaves unset, so the primary user is always `superadmin`.
// Super-admin is global — it can read and write across every tenant — so `auth setup`
// always mints a platform-wide account. Use `mercato auth add-user --roles admin` to
// create a tenant-scoped user. Changing this default would alter super-admin behavior
// and tenant-provisioning outputs, which packages/core/src/modules/auth/AGENTS.md marks
// "ask first"; the warning emitted below makes the current behavior explicit instead.
const SETUP_USAGE = 'Usage: mercato auth setup --orgName <name> --email <email> --password <password> [--orgSlug <slug>] [--roles superadmin,admin,employee (roles to CREATE; the primary user is always superadmin)] [--skip-password-policy] [--include-demo-users] [--with-examples] [--json]'
...
const setupApp: ModuleCli = {
  ...
  async run(rest) {
    ...
    // `--roles` chooses which roles to CREATE; the primary user is always seeded as
    // `superadmin` (see SETUP_USAGE). An operator who passes an explicit role list
    // without `superadmin` is almost certainly trying to provision a tenant-scoped
    // account, and would otherwise silently receive a GLOBAL super-admin able to read
    // and write every other tenant's data. Say so loudly rather than changing the
    // provisioning semantics behind their back.
    if (typeof args.roles === 'string' && roleNames && !roleNames.includes('superadmin')) {
      process.stderr.write(
        `WARNING: --roles ${roleNames.join(',')} controls which roles are CREATED, not which roles the primary user gets.\n` +
        `         ${email} will still be created as a GLOBAL superadmin with cross-tenant read/write access.\n` +
        `         To create a tenant-scoped user instead, run:\n` +
        `           mercato auth add-user --email <email> --password <password> --organizationId <id> --roles ${roleNames.join(',')}\n`,
      )
    }
