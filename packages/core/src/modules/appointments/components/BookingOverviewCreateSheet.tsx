...
function normalizePhoneValue(phone: string | null | undefined, phoneCountryCode: string | null | undefined) {
  const value = phone?.trim() ?? ''
  if (!value || value.startsWith('+')) return value
  return `${phoneCountryCode ?? '+84'} ${value}`
}
...
function isDropdownOverlayTarget(target: EventTarget | null) {
  return target instanceof Element && Boolean(target.closest('[data-slot="dropdown-menu"]'))
}
...
export function BookingOverviewCreateSheet({
  ...
  const [phoneCountryCode, setPhoneCountryCode] = React.useState('+84')
  const [phoneCountry, setPhoneCountry] = React.useState('vn')
  const [sheetContentElement, setSheetContentElement] = React.useState<HTMLElement | null>(null)
  ...
  React.useEffect(() => {
    if (!open) return
    setPhone(''); setPhoneCountryCode('+84'); setPhoneCountry('vn'); setName(''); setEmail(''); setSalutation('None'); setOrigin(''); setSource(''); setBookingType('walk_in'); setCustomerSearch(''); setCustomerSearchResults([]); setIsCustomerSearchOpen(false); setInternalNotes(''); setExternalNotes(''); setSelectedServices([]); setPreviousServices([]); setStep('customer')
  }, [open])
  ...
  const applyCustomer = async (result: CustomerSearchResult) => {
    ...
    setPhone(normalizePhoneValue(customer.phone, customer.phoneCountryCode))
    setPhoneCountryCode(customer.phoneCountryCode ?? '+84')
    setPhoneCountry(customer.phoneCountry ?? 'vn')
  ...
  const lookupCustomer = async () => {
    ...
    try {
      const call = await apiCall<{ exists?: boolean; customer?: Customer }>('/api/customers/people/check', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ tenantId, phone: phone.trim(), phoneCountryCode, phoneCountry }),
      }, { fallback: null })
      ...
      setName(customer.name ?? ''); setEmail(customer.email ?? ''); setPhone(normalizePhoneValue(customer.phone ?? phone, customer.phoneCountryCode ?? phoneCountryCode)); setPhoneCountryCode(customer.phoneCountryCode ?? phoneCountryCode); setPhoneCountry(customer.phoneCountry ?? phoneCountry); setCustomerSearch(customer.name ?? customer.phone ?? phone); setSalutation(customer.salutation ?? 'None')
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent ref={setSheetContentElement} side="right" className="w-full gap-0 overflow-hidden sm:max-w-md" onPointerDownOutside={(event) => { if (isDropdownOverlayTarget(event.detail.originalEvent.target)) event.preventDefault() }} onFocusOutside={(event) => { if (isDropdownOverlayTarget(event.detail.originalEvent.target)) event.preventDefault() }}>
    ...
    <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col overflow-y-auto px-6 py-4">
      ...
      {step === 'customer' ? <div className="space-y-4">
        ...
        <div className="space-y-1.5"><Label>{t('appointments.create.field.phone', 'Phone')} <span className="text-status-error-text">*</span></Label><div className="flex gap-2"><div className="min-w-0 flex-1"><PhoneNumberField value={phone || null} onValueChange={(next) => setPhone(next ?? '')} onCountryChange={(country) => { setPhoneCountryCode(country.dialCode); setPhoneCountry(country.iso2.toLowerCase()) }} placeholder={t('appointments.overview.create.phone', 'Phone number')} invalidLabel={t('appointments.create.field.phone.invalid', 'Enter a valid phone number.')} minDigits={7} defaultCountryIso2="VN" dropdownElevated dropdownPortalContainer={sheetContentElement} fieldClassName="bg-input-bg" /></div><Button type="button" variant="outline" size="icon" onClick={() => void lookupCustomer()} disabled={isLookingUp}>{isLookingUp ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}</Button></div></div>
