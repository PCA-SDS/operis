...
/**
 * Public contract for `wms`.
 *
 * Warehouse and inventory. Reacts to `sales.order.*`; its own writes are driven by its API.
 *
 * Everything under `data/`, `lib/`, `services/`, `components/` and `api/` is
 * private: `packages/core/package.json` maps `./modules/wms/*` to null, so a
 * cross-module deep import fails to resolve at compile time rather than in review.
 *
 * This contract is deliberately empty. No other module consumes wms today, and
 ...
 *
 *   commands:   ['wms.<entity>.<action>']   // cross-module writes, via the command bus
 *   readModels: ['wms:<entity>']            // cross-module reads, via the query engine
 *
