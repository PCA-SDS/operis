...
export default function EditCatalogProductPage({ params }: { params?: { id?: string } }) {
    ...
    const productSubpathPrefix = productId ? `/backend/catalog/products/${productId}/` : null;
    ...
    const [initialValues, setInitialValues] = React.useState<Partial<ProductFormValues> | null>(
        null,
    );
    ...
    const [variantMediaGroups, setVariantMediaGroups] = React.useState<VariantMediaGroup[]>([]);
    ...
    const fetchTaxRateById = React.useCallback(
        async (taxRateId: string): Promise<TaxRateSummary | null> => {
            const payload = await readApiResultOrThrow<{
                items?: Array<Record<string, unknown>>;
            }>(`/api/sales/tax-rates?id=${encodeURIComponent(taxRateId)}&pageSize=1`, undefined, {
                errorMessage: t(
        ...
        async function loadProduct() {
            ...
            try {
                ...
                const initial: ProductFormValues = {
                    ...
                    productType: (typeof record.product_type === "string"
                        ? record.product_type
                        : typeof record.productType === "string"
                          ? record.productType
                          : "simple") as CatalogProductType,
        ...
        () => [
            {
                ...
                component: ({ values, setValue, errors, requiredFieldIds }) => (
                    <ProductDetailsSection
                        ...
                        productId={productId ?? ""}
                        hasVariants={Boolean((values as ProductFormValues).hasVariants)}
        ...
        async (formValues: ProductFormValues) => {
            ...
            if (!parsed.success) {
                ...
                const message =
                    ...
                    t("catalog.products.edit.errors.validation", "Fix highlighted fields.");
            ...
            let defaultMediaId =
                typeof values.defaultMediaId === "string" && values.defaultMediaId.trim().length
            ...
            const unitPriceReferenceUnit = canonicalizeUnitCode(values.unitPriceReferenceUnit);
            ...
            const payload: Record<string, unknown> = {
                ...
                isConfigurable: isConfigurableProductType(values.productType || "simple"),
                metadata,
                dimensions,
            ...
            <Page>
                <PageBody>
                    <RecordNotFoundState
                        ...
                        backLabel={t(
                            "catalog.products.edit.actions.backToList",
                            "Back to products",
                        )}
                    />
                </PageBody>
...
function ProductOptionsSection({ values, setValue }: ProductFormGroupProps) {
    ...
    const [schemaToEdit, setSchemaToEdit] = React.useState<OptionSchemaTemplateSummary | null>(
        null,
    );
    ...
    const handleDeleteSchema = React.useCallback(
        async (id: string) => {
            ...
            try {
                await withScopedApiRequestHeaders(buildOptimisticLockHeader(lockVersion), () =>
        ...
        <>
            <div className="space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                    <h3 className="text-sm font-semibold">
                        {t("catalog.products.create.optionsBuilder.title", "Product options")}
                    </h3>
            ...
            <div className="space-y-3">
                        ...
                        {values.productType === "service" || values.productType === "bundle" ? (
                            <>
                                <Button asChild size="sm" variant="outline">
                                    <Link href={`/backend/catalog/products/${productId}/options`}>
                                        {t(
                                            "catalog.products.edit.options.manage",
                                            "Manage Options",
                                        )}
                                    </Link>
                                </Button>
                                <Button asChild size="sm" variant="outline">
                                    ...
                                    >
                                        {t(
                                            "catalog.products.edit.constraints.manage",
                                            "Manage Constraints",
                                        )}
                                    </Link>
                                </Button>
                ...
                {variants.length ? (
                    <div className="overflow-x-auto rounded-md border">
                        <Table
                            columns={["minmax(0, 1fr)", "10rem", "12rem", "6rem", "10rem"]}
                            density="compact"
                                        ...
                                        <TableCell align="right">
                                            <div className="flex justify-end gap-2 whitespace-nowrap">
                                                <Button asChild size="sm" variant="outline">
                                                    ...
                                                    >
                                                        {t(
                                                            "catalog.products.list.actions.edit",
                                                            "Edit",
                                                        )}
                                                    </Link>
                                                </Button>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                    ...
                    <Button type="button" variant="outline" onClick={handleGenerateHandle}>
            ...
            <div className="space-y-2">
                ...
                <Input
                    ...
                    placeholder={t("catalog.products.create.placeholders.sku", "e.g., PROD-001")}
                    className="font-mono"
                />
                <p className="text-xs text-muted-foreground">
                    {t(
            ...
            <div className="space-y-2">
                <Label>{t("catalog.products.form.productType", "Product type")}</Label>
                <Select
                ...
                {errors.productType ? (
                    <p className="text-xs text-status-error-text">{errors.productType}</p>
                ) : null}
            </div>
                ...
                <div className="flex items-center justify-between gap-2">
                    ...
                    >
                        ...
                        <span className="sr-only">
                            {t("catalog.products.create.taxRates.manage", "Manage tax classes")}
                        </span>
                    </Button>
                </div>
                <Select
...
function readCategorySelections(record: Record<string, unknown>): {
    ...
    const options = rawCategories
        .map((entry) => {
            const value =
                getString(entry.id) ?? getString(entry.categoryId) ?? getString(entry.category_id);
...
function readOfferSnapshots(record: Record<string, unknown>): OfferSnapshot[] {
    ...
    for (const offer of rawOffers) {
        ...
        const channelId = getString(offer.channelId) ?? getString(offer.channel_id) ?? null;
...
function buildChannelOptions(offers: OfferSnapshot[]): ProductCategorizePickerOption[] {
    ...
    for (const offer of offers) {
        ...
        const description =
            offer.channelCode && offer.channelCode !== label ? offer.channelCode : null;
        options.push({
...
function extractUomFields(record: Record<string, unknown>) {
    const unitPriceObject = isRecord(record.unit_price)
        ...
        : null;
    const defaultUnit = canonicalizeUnitCode(record.default_unit ?? record.defaultUnit);
    ...
    const roundingModeRaw = toTrimmedOrNull(record.uom_rounding_mode ?? record.uomRoundingMode);
