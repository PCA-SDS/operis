...
describe('SalesDocumentItemsSection discount column', () => {
  ...
  it('leaves the cell empty on lines that carry no discount', async () => {
    ...
    expect(bodyRows[0].querySelectorAll('[role="cell"]')[columnIndex]?.textContent).toBe(`−${money(4.5)}`)
    expect(bodyRows[1].querySelectorAll('[role="cell"]')[columnIndex]?.textContent).toBe('')
  })
})
