...
export class Migration20260825105741_invoice extends Migration {
  ...
  override up(): void | Promise<void> {
    ...
    this.addSql(`create unique index "invoice_companies_tax_code_scope_unique_idx" on "invoice_companies" ("organization_id", "tenant_id", "tax_code") where deleted_at is null;`);
    ...
    this.addSql(`create unique index "invoice_invoices_email_tracking_hash_unique_idx" on "invoice_invoices" ("email_tracking_token_hash") where email_tracking_token_hash is not null and deleted_at is null;`);
    this.addSql(`create unique index "invoice_invoices_source_scope_unique_idx" on "invoice_invoices" ("organization_id", "tenant_id", "source_invoice_id") where deleted_at is null;`);
