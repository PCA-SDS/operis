...
export function InvoiceSyncDialog({ open, onOpenChange, availability, initialJob, onJobChange, onCompleted }: { open: boolean; onOpenChange: (open: boolean) => void; availability: SyncAvailability; initialJob: SyncJob | null; onJobChange: (job: SyncJob) => void; onCompleted: () => void }) {
  ...
  const start = async () => {
    ...
    try {
      ...
      else if (result.state === 'cooldown') setError(t('invoice.sync.errors.cooldown', 'Please wait {seconds} seconds before syncing again.', { seconds: result.retryAfterSeconds }))
  ...
  const authenticate = async () => {
    ...
    try {
      ...
      else if (result.state === 'retry') { if (result.transactionId) setTransactionId(result.transactionId); if (result.captchaSvg) setCaptchaSvg(result.captchaSvg); setError(t('invoice.sync.errors.authRetry', 'The portal rejected the sign-in. Try the new CAPTCHA. {count} attempts remain.', { count: result.attemptsRemaining ?? 0 })) }
      else if (result.state === 'account_locked') setError(result.message ?? t('invoice.sync.errors.accountLocked', 'The tax portal account is locked.'))
      else setError(t('invoice.sync.errors.authCooldown', 'Too many attempts. Try again in {seconds} seconds.', { seconds: result.retryAfterSeconds }))
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!busy) onOpenChange(next) }}><DialogContent size="lg" dismissible={!busy} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === 'Enter' && canSubmit && !busy) { event.preventDefault(); submit() } }}>
    ...
    <DialogBody className="space-y-5">{error ? <Alert status="error" style="lighter">{error}</Alert> : null}
      ...
      {step === 'authenticate' ? <><Alert status="information" style="lighter">{t('invoice.sync.secretNotice', 'Your password and CAPTCHA are used only for this sign-in and are not stored in browser application state.')}</Alert>{captchaSvg ? <img src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(captchaSvg)}`} alt={t('invoice.sync.captchaAlt', 'Tax portal CAPTCHA')} className="h-20 max-w-full rounded-lg border border-border bg-surface p-2" /> : null}<div className="space-y-2"><Label htmlFor="invoice-sync-password">{t('invoice.sync.password', 'Tax portal password')}</Label><Input id="invoice-sync-password" type="password" autoComplete="off" value={password} onChange={(event) => setPassword(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="invoice-sync-captcha">{t('invoice.sync.captcha', 'CAPTCHA')}</Label><Input id="invoice-sync-captcha" autoComplete="off" value={captchaSolution} onChange={(event) => setCaptchaSolution(event.target.value)} /></div></> : null}
      {step === 'progress' ? <SyncProgress job={job} t={t} /> : null}</DialogBody>
    <DialogFooter leading={step === 'progress' && !terminal ? <span className="text-sm text-muted-foreground">{t('invoice.sync.backgroundNotice', 'You can close this dialog. Progress remains in the top bar.')}</span> : undefined}>{step === 'authenticate' ? <Button type="button" variant="outline" disabled={busy} onClick={() => { setPassword(''); setCaptchaSolution(''); setError(null); setStep('configure') }}>{t('invoice.sync.back', 'Back')}</Button> : null}{step !== 'progress' ? <Button type="button" disabled={!canSubmit || busy} onClick={submit}>{busy ? <Spinner /> : null}{step === 'authenticate' ? t('invoice.sync.authenticate', 'Sign in and sync') : t('invoice.sync.continue', 'Continue')}</Button> : null}{job?.state === 'FAILED' ? <Button type="button" onClick={() => { setError(null); setJob(null); setStep('configure'); setIdempotencyKey(createIdempotencyKey()) }}>{t('invoice.sync.retry', 'Try again')}</Button> : step === 'progress' ? <Button type="button" onClick={() => { onOpenChange(false); if (terminal) { setStep('configure'); setIdempotencyKey(createIdempotencyKey()) } }}>{terminal ? t('invoice.sync.done', 'Done') : t('invoice.sync.background', 'Run in background')}</Button> : null}</DialogFooter>
