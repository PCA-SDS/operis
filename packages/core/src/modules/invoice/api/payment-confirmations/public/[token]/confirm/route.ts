...
export async function POST(req: Request, routeContext: RouteContext = {}) {
  try {
    ...
    const result = await prepared.service.confirmPublic(prepared.token)
    return NextResponse.json(invoicePaymentConfirmationPublicTransitionSchema.parse(result))
  } catch (error) {
    if (isCrudHttpError(error)) return NextResponse.json(error.body, { status: error.status })
    logger.error('Public payment confirmation confirm failed', { err: error })
...
export const openApi: OpenApiRouteDoc = {
  ...
  summary: 'Confirm a payment confirmation',
  methods: {
    POST: {
      operationId: createInvoiceOperationId('paymentConfirmationPublic', 'confirm'),
