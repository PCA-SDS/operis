...
/**
 * A network-free carrier used ONLY by tests and local development.
 *
 * It lived in the app's `example` module until that module was dropped from the
 * build (`d108ad27`, "drop development-only modules"), which took the
 * `mock_carrier` provider with it and left 19 integration specs calling a
 * carrier nothing registered any more — surfacing as
 * `502 Carrier provider request failed`. A test fixture for this module belongs
 * in this module, not in a product module that can be switched off.
 *
 * **It is never registered at import.** `ensureMockCarrierRegistered()` is the
 * only entry point and no-ops unless `OM_ENABLE_MOCK_SHIPPING_CARRIER` is set.
 */
