import { packIntervals } from '../layout'
...
function packedById(packed: ReturnType<typeof packIntervals<CalendarItem>>) {
  return new Map(packed.map((packedEntry) => [packedEntry.entry.id, packedEntry]))
}
...
describe('packIntervals', () => {
  ...
  it('orders equal starts by longer duration first', () => {
    ...
    expect(packed[0].entry.id).toBe('long')
    expect(packed[0]).toMatchObject({ column: 0, columns: 2 })
    expect(packed[1].entry.id).toBe('short')
