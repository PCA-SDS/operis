 ...
 * search), the category filter ("All Scheduled" / "Meetings" / "Events"
 * radiogroup, All active by default), view switcher (Day/Week/Month/Agenda
 * radiogroup, Week active by default), shortcut footer
...
test.describe('TC-CAL-002: Calendar page load & week grid hydration', () => {
  test('page shell, week grid block and sidebar entry render after hydration', async ({ page, request }) => {
    ...
    try {
      ...
      await expect(categoryFilter.getByRole('radio', { name: /^Meetings \(\d+\)$/ })).toBeVisible();
      await expect(categoryFilter.getByRole('radio', { name: /^Events \(\d+\)$/ })).toBeVisible();
