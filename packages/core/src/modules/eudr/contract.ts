...
/**
 * Public contract for `eudr`.
 *
 * Everything under `data/`, `lib/`, `services/`, `components/` and `api/` is
 * private: the package exports map sends `./modules/eudr/*` to null, so a
 * cross-module deep import fails to resolve at compile time rather than in review.
 *
 * Empty because no other module consumes eudr today. Publishing a surface
 ...
 *
 *   commands:   ['eudr.<entity>.<action>']   // cross-module writes, via the command bus
 *   readModels: ['eudr:<entity>']            // cross-module reads, via the query engine
 *
