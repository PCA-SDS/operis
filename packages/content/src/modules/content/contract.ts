...
/**
 * Public contract for `content`.
 *
 * Everything under `data/`, `lib/`, `services/`, `components/` and `api/` is
 * private: the package exports map sends `./modules/content/*` to null, so a
 * cross-module deep import fails to resolve at compile time rather than in review.
 *
 * Empty because no other module consumes content today. Publishing a surface
 ...
 *
 *   commands:   ['content.<entity>.<action>']   // cross-module writes, via the command bus
 *   readModels: ['content:<entity>']            // cross-module reads, via the query engine
 *
