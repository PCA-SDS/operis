...
test('build emits customers facts and the framework extension catalog (T5)', () => {
  ...
  // Upstream additionally asserted the v1 -> v2 `component-override` mode correction
  // ('replace' -> 'wrapper') using the enterprise `security` module's
  // `section:auth.login.form` contribution. That module is commercially licensed and is
  // not part of this fork (see NOTICE.md), so `facts.security` is undefined here.
  //
  // No OSS module reproduces a v1/v2 mode divergence — verified by extracting both
  // contract versions across every discovered module and finding zero differing
  // component-override contributions. The equivalent guard in
  // packages/cli/src/lib/generators/__tests__/module-facts.bc-guard.test.ts keeps
  // covering the v1/v2 divergence through the core `catalog` module.
  //
  // The customers/framework assertions this test is named for are above; `v2Facts` is
  // still exercised there, so dropping the security block costs no OSS coverage.
})
