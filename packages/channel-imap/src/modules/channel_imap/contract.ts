...
/**
 * Public contract for `channel_imap`.
 *
 * Everything under `data/`, `lib/`, `services/`, `components/` and `api/` is
 * private: the package exports map sends `./modules/channel_imap/*` to null, so a
 * cross-module deep import fails to resolve at compile time rather than in review.
 *
 * Empty because no other module consumes channel_imap today. Publishing a surface
 ...
 *
 *   commands:   ['channel_imap.<entity>.<action>']   // cross-module writes, via the command bus
 *   readModels: ['channel_imap:<entity>']            // cross-module reads, via the query engine
 *
 * Events are public by default and bind by id string — see `events.ts`, not here.
 */
export const contract = defineModuleContract({
  moduleId: 'channel_imap',
})
